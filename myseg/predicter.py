...
class Predicter:
    def __init__(self, config: PredicterConfig):
        self.config = PredicterConfig

    def load_model_from_trainer_config(
        self, train_config: TrainerConfig
    ) -> torch.nn.Module:
    ...
    def _build_test_time_preprocess(
        self, pred_width: int, pred_height: int, model_config: ModelConfig
    ):
    ...
    def _wrap_model_if_use_tta(
        self, model: torch.nn.Module, predict_config: PredicterConfig
    ):
        if predict_config.USE_TTACH:
            model = ttach.wrappers.SegmentationTTAWrapper(
                model, predict_config.test_time_augmentation_compose
            )
            return model
    ...
    def predict(
        ...
        predict_config: PredicterConfig,
    ) -> np.ndarray:
        """
        ...
        preprocess = self._build_test_time_preprocess(
            predict_config.TARGET_WIDTH, predict_config.TARGET_HEIGHT, model_config
        )
        images = [preprocess(image=img)["image"] for img in images]
        preds = []

        padding_dict = calculate_padding_size_by_augmentation(
            origin_h=original_height,
            origin_w=original_width,
            target_h=predict_config.TARGET_HEIGHT,
            target_w=predict_config.TARGET_WIDTH,
        )
        self._wrap_model_if_use_tta()
        model.eval()
        for i in tqdm(range(0, len(images), predict_config.BATCH_SIZE)):
            tensor = torch.from_numpy(
                np.stack(images[i : i + predict_config.BATCH_SIZE])
            )
            # _, _, origin_height, origin_width = tensor.shape
            tensor = tensor.to(predict_config.DEVICE)
            ...
            original_size_pred = resize_batch_image(
                ...
                resize_interpolation=predict_config.RESIZE_INTERPOLATION,
    ...
    def predict_from_folder(
        ...
        return self.predict(images, model, model_config, predict_config)
